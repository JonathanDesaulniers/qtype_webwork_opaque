...
define("qtype_webwork_opaque/knowl", ["jquery"], function(a) {
    var b = {
        init: function init() {
	...
	const initializeKnowl = (knowl) => {
		...
		if (!knowl.knowlContainer) {
			...
			knowl.knowlContainer.addEventListener('show.collapse, show.bs.collapse', () => knowl.classList.add('active'));
			knowl.knowlContainer.addEventListener('hide.collapse, hide.bs.collapse', () => knowl.classList.remove('active'));
			...
			knowl.dataset.target = `#${knowl.knowlContainer.id}`;
