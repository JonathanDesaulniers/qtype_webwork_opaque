define("qtype_webwork_opaque/dragndrop", ["jquery","qtype_webwork_opaque/jquery-nestable"], function() {
...
// Fix pour les id avec un ":"
			$("#" +  this.answerInputId.replace(":", "\\:")).val(buckets.join(","));
		}
	...
	$('div.dd-bucket-pool').each(function() {
		...
		// Force use of opaque prefix for Moodle 
		const dataAns = $(this).attr('data-ans');
		const answerInputId = qPrefix + dataAns;
		$(this).parent().children("input[name$='" + dataAns + "']").attr({id: answerInputId, name:answerInputId});
		
		$(this).attr('data-ans', answerInputId);
