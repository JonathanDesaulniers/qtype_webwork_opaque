...
window.graphTool = (containerId, options) => {
	...
	const AnsOnly = containerId.replace("_graphbox", "");
	const answerInputId = qPrefix + AnsOnly;
	const previousanswerInputId = qPrefix + 'previous_' + AnsOnly;
	$(gt.graphContainer).parent().children("input[name='" + AnsOnly + "']").attr({id: answerInputId, name:answerInputId});
	$(gt.graphContainer).parent().children("input[name='previous_" + AnsOnly + "']").attr({id: previousanswerInputId, name:previousanswerInputId});
	/*$(this).document.getElementsByName(AnsOnly).attr({id: qPrefix + AnsOnly, name: qPrefix + AnsOnly});*/
	/*const NameCont1 = document.getElementsByName(AnsOnly);
	NameCont1.forEach(input => {
		input.id = qPrefix + AnsOnly; 
		input.name = qPrefix + AnsOnly;
	})
	const NameCont2 = document.getElementsByName('previous_' + AnsOnly);
	NameCont2.forEach(input => {
		input.id = qPrefix + 'previous_' + AnsOnly; 
		input.name = qPrefix + 'previous_' + AnsOnly;
	})*/
	...
	if (!gt.isStatic) {
		...
		gt.tooltips = Array.from(
			document.querySelectorAll('.gt-button-div[data-toggle="tooltip"],.gt-button[data-toggle="tooltip"]'))
			.map((tooltip) => new tooltip(tooltip,
